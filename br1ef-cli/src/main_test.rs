use super::*;
use br1ef_core::Item;
use br1ef_core::Source;
use br1ef_core::config::AppConfig;
use br1ef_core::storage::{InMemoryStorage, Storage};

fn storage_with(items: &[(&str, Source, &str, &str)]) -> InMemoryStorage {
    let mut s = InMemoryStorage::new();
    let stored: Vec<Item> = items
        .iter()
        .map(|&(id, ref source, title, mailbox)| Item {
            id: id.into(),
            title: title.into(),
            from: "sender".into(),
            body: "body".into(),
...
fn count_items_single_mailbox() {
    let storage = storage_with(&[("a", Source::Imap, "title a", "INBOX")]);
...
fn count_items_multiple_mailboxes() {
    let storage = storage_with(&[
        ("a", Source::Imap, "post a", "INBOX"),
        ("b", Source::Imap, "update b", "Updates"),
        ("c", Source::Imap, "post c", "Social"),
        ("d", Source::Imap, "thread d", "Social"),
    ]);
...
fn count_items_multiple_sources_with_mailboxes() {
    let storage = storage_with(&[
        ("a", Source::Imap, "inbox mail", "INBOX"),
        ("b", Source::Imap, "social post", "Social"),
        ("c", Source::Imap, "channel msg", "general"),
        ("d", Source::Imap, "another social", "Social"),
        ("e", Source::Imap, "announcement", "announcements"),
    ]);
...
fn count_items_empty_mailbox_displayed_as_unknown() {
    let storage = storage_with(&[("a", Source::Imap, "title a", "")]);
...
fn count_items_gmail_category_strips_prefix() {
    let storage = storage_with(&[("a", Source::Imap, "social mail", "@@CATEGORY@@/Social")]);
...
fn list_items_shows_items() {
    let storage = storage_with(&[("a", Source::Imap, "hello world", "INBOX")]);
...
fn list_items_truncates_long_titles() {
    ...
    let storage = storage_with(&[("a", Source::Imap, &long_title, "INBOX")]);
...
fn list_items_short_title_not_truncated() {
    let storage = storage_with(&[("a", Source::Imap, "short title", "INBOX")]);
...
fn list_items_multiple_items() {
    let storage = storage_with(&[
        ("a", Source::Imap, "first item", "INBOX"),
        ("b", Source::Imap, "second item", "INBOX"),
    ]);
